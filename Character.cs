...
public class Monster : Character // Monster 캐릭터 관리
{
    ...
    public Monster(int id) // MonsterType 1번은 잡몹, 2번은 레어몹, 3번은 보스몹
    {
        ...
        }   // 추가설명: 녹슨 아밍소드 Atk= 0.5f 상점에 팔면 될거 같다. 
        else if (id == 5)
        {
            this.MonsterType = 2;
            this.MonsterID = id;
        ...
        }   // 추가설명: 와일드보어 가죽갑옷 Def= 1.5f 와일드 보어 가죽으로 무두질한 가죽갑옷이다 가죽이 질겨서 베는 공격에 저항이높다.
        else if (id == 6)
        {
            this.MonsterType = 1;
            this.MonsterID = id;
        ...
        else if (id == 17) //애착 몬스터
        {
            ...
            //추가설명: n, n, n, 확률 드랍";
            //천산갑 비늘 어린갑 Def=  천산갑이라는 동물의 비늘을 겹겹이 쌓아서 안감과 연결해서 만들었다 천산갑의 비늘은 상당히 고가이며 내구성도 매우 높아 부르는 게 값이다. 
        }
        else if (id == 18)
        {
            this.MonsterType = 2;
            ...
            //추가설명: n, n, n, 확률 드랍";
            //역전의 마티네 목걸이 Atk=  쌍검이 크로스로 겹쳐있는 모양이 달려있는 황금 목걸이다, 착용하면 힘이 솟아난다.
        ...
        else if (id == 31)
        {
            ...
            this.MaxHP = 150;
            this.NowHP = 150;
            this.Details = "설명: 다크 본 스태프, 심연의 보주, 죽은 자의 로브 장착,몸은 미라처럼 삐쩍 마르고 두 눈과 벌린 입에서는 검은색 연기가 피어오른다."; //\n 임시
        ...
        else if (id == 32)
        {
            ...
            this.MonsterID = id;
            this.Level = 30;
            this.Name = " 붉은 바폰메트인 벨쥬(Boss)[등급: 예측불가]"; // HardScreen
            this.Atk = 31;
            this.Def = 16;
            this.MaxHP = 220;
            this.NowHP = 220;
            this.Details = "설명: 일격의 파르티잔, 레드빈 인풋 망토, 팔콘경번갑 장착, 바폰메트는 악마종의 하나다 누구도 감히 도전할 수 없는 존재 그는 산양인과 비슷하지만 훨씬 거대하며, 불길한 붉은 털과 거대한 뿔을 지닌 악마 그 자체다.";
            // 설명: 던전에는 아인종 중에 하나인 산양인 비스트맨이 무리 지어 살고있다. 기본 2미터 크기이며 외형은 머리 산양 상반신 인간 하반신 산양의 다리, 온몸에 흰색 털이 나있다.
            // 설명: 던전 깊숙한 곳에 악마종인 바폰메트인 산양인을 통치하고 있다는 소문이 있다 그게 사실이라면 싸우지 말고 도망가는 것이 좋겠다.
            // 스킬: 
        }
        else
        {
